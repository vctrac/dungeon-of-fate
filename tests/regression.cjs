...
(async()=>{
 ...
 assert.match(await page.title(),/V2\.19/);assert.equal((await state()).hp,3);
 ...
 for(const [dx,dy] of directions){
  ...
  assert.equal((await state()).combo,2,'Preview must not grant FATE');
 ...
 assert.equal(await page.evaluate(()=>caches.keys().then(keys=>keys.includes('dungeon-of-fate-v2.19.1-1'))),true);
