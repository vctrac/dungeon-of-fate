...
(async()=>{
 ...
 assert.match(await page.title(),/V2\.17/);assert.equal((await state()).hp,3);
 ...
 assert.equal(await page.evaluate(()=>caches.keys().then(keys=>keys.includes('dungeon-of-fate-v2.17.1-1'))),true);
