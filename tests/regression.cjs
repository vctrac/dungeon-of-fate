...
(async()=>{
 ...
 assert.match(await page.title(),/V2\.18/);assert.equal((await state()).hp,3);
 ...
 assert.equal(await page.evaluate(()=>caches.keys().then(keys=>keys.includes('dungeon-of-fate-v2.18.1-1'))),true);
