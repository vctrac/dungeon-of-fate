module.exports=()=>{
  ...
  for(const depth of [1,2,5,10,20,40]){
   ...
   for(let i=0;i<80;i++){
    t.setCombo(12);t.setFloor(depth);const s=state(),active=s.rooms.filter(r=>r.active&&r.id!==s.startId&&r.id!==s.exitId&&(!s.fateGate||!s.fateGate.branchIds.includes(r.id)));
