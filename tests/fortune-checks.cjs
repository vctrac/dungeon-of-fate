module.exports=()=>{
  ...
  for(const [kind,roll,expected] of [['basic',1,5],['basic',2,5.5],['spirit',1,1],['spirit',2,4],['spirit',3,5],['spirit',4,6],['spirit',5,6],['spirit',6,6.7]]){
  ...
  t.newRun();t.setCombo(6);t.acquireTrinket('doll');t.resolveDice('monster',1);check(state().hp===3&&state().combo===1&&state().relics.floorUsed.doll,'doll');t.clearPending();
  t.newRun();t.setVitals(2,false);t.acquireTrinket('blood');t.resolveDice('monster',5);check(state().hp===3&&state().relics.floorUsed.blood,'blood');t.clearPending();
  t.newRun();t.offerConsumable('charm');check(t.useConsumable(false)&&state().shield&&!state().relics.consumable,'charm');t.resolveDice('monster',1);check(state().hp===3&&!state().shield,'shield');t.clearPending();
  t.newRun();t.offerConsumable('bargain');check(t.useConsumable(true)&&state().hp===1&&state().relics.bargainCharges===3,'bargain activation');t.resolveDice('monster',1);check(state().hp===1,'bargain protection');t.clearPending();
  for(const type of ['monster','trap','heal']){
   t.newRun();t.resolveDice(type,6);let plain=state().gold;t.clearPending();t.newRun();t.acquireTrinket('horseshoe');t.resolveDice(type,6);check(state().gold-plain===30,'horseshoe');t.clearPending();
  }
  ...
  for(let i=1;i<=300;i++){
   ...
   seed(i);t.setCombo(7);t.generateDungeon();const b=state();check(b.floorFortune===7&&b.fortuneBudget<=5&&b.fortuneUpgrades.length<=3,'cap/snapshot');
  ...
  ids.forEach((id,i)=>t.setRoomFixture(id,{event:'monster',monsterKind:['basic','thief','spirit'][i]}));t.acquireTrinket('eye');
  ...
  // Paired generous five-floor reward sweep: every room, every scavenge, no deaths.
  let wallets=[];
  for(let run=0;run<30;run++){
   ...
   for(let floor=1;floor<=5;floor++){
    ...
    for(const r of s.rooms.filter(r=>r.active&&r.id!==s.startId&&r.id!==s.exitId)){
     ...
     t.setRoomEvent(r.id,r.event);t.scavenge(r.id,.8);t.clearPending();
    }
   }
   const s=state();check(s.gold<10000,'early inflation');check(s.floorEconomy.entering+s.floorEconomy.gained-s.floorEconomy.lost===s.gold,'telemetry');wallets.push(s.gold);
  }
  return{pairedFloors:300,low,high,rich,variants:signatures.size,generousFiveFloorGold:{min:Math.min(...wallets),max:Math.max(...wallets),mean:Math.round(wallets.reduce((a,b)=>a+b)/wallets.length)}};
 };
