...
try{
 ...
 await page.locator('#itemChoice').tap({position:{x:5,y:5}});assert.equal((await state()).cards.active,null);assert(!(await state()).rooms[altar.id].altarUsed);
 ...
 await page.locator('#itemChoice').tap({position:{x:5,y:5}});await page.evaluate(()=>{__dofTest.acquireTrinket('doll');__dofTest.closeItemCard();__dofTest.openAltar(__dofTest.state().currentId)});
 ...
 for(const [hp,fate] of [[3,5],[1,1]]){altar=await findAltar();await page.waitForFunction(()=>__dofTest.state().cards.active?.type==='altar');await page.locator('#itemChoice').tap({position:{x:5,y:5}});await page.evaluate(({hp,fate})=>{__dofTest.setVitals(hp,false);__dofTest.setCombo(fate);__dofTest.openAltar(__dofTest.state().currentId)},{hp,fate});assert(await page.locator('#altarOffer').isDisabled());assert.equal(await page.evaluate(()=>__dofTest.sacrificeAltar()),false);await page.locator('#itemChoice').tap({position:{x:5,y:5}});assert.equal((await state()).hp,hp);assert.equal((await state()).combo,fate)}
 ...
 for(const id of ['eye','blood','doll']){await page.evaluate(id=>__dofTest.acquireTrinket(id),id);assert(await page.locator('#closeItem').isHidden());await page.locator('#itemChoice').tap({position:{x:5,y:5}})}
 await page.evaluate(()=>__dofTest.acquireTrinket('horseshoe'));await page.locator('#itemName').tap();assert((await state()).cards.active.decision);await page.locator('#itemChoice').click({position:{x:5,y:5}});
 for(const size of [{width:320,height:568},{width:844,height:390}]){await page.setViewportSize(size);altar=await findAltar();await page.waitForFunction(()=>__dofTest.state().cards.active?.type==='altar');const bounds=await page.locator('.itemChoicePanel').boundingBox();assert(bounds.x>=0&&bounds.y>=0&&bounds.x+bounds.width<=size.width&&bounds.y+bounds.height<=size.height);const target=await page.locator('#altarOffer').boundingBox();assert(target.height>=80);assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth&&document.documentElement.scrollHeight<=innerHeight));await page.locator('#itemChoice').tap({position:{x:5,y:5}})}
